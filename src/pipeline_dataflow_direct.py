...
class ParseDataDoFn(beam.DoFn):
    def __init__(self):
        ...
        self.last_valid_values = (
            {}
        )  # To store the last valid value for each key (including 'Date')
    ...
    def _fill_missing_values(self, row):
        ...
        if "Unemployment" in row:
            ...
            row["Unemployment"] = self._fill_with_last_valid(
                "Unemployment", row.get("Unemployment"), converter=float
            )
...
def merge_records(records):
    ...
    key, grouped_records = (
        records  # key is the common key (index), grouped_records is a dict
    )
...
def format_csv_row_clean(record):
    ...
    if not format_csv_row_clean.header_added:
        csv_output.append(
            "index,Store_ID,Month,Dept,IsHoliday,Weekly_Sales,CPI,Unemployment"
        )
...
def format_csv_row_agg(record):
    ...
    # Create a row in CSV format
    row = (
        f"{format_csv_row_agg.index},{record['Month']},{record['Average_Weekly_Sales']}"
    )
    csv_output.append(row)
...
def run_pipeline():
    pipeline_options = PipelineOptions(runner="DirectRunner")  # Run locally
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        csv_pcoll = (
            ...
            >> beam.io.ReadFromText(
                "/Users/nicolas/repos/walmart/data/grocery_sales.csv",
                skip_header_lines=1,
            )
            | "Parse CSV and Fill Nulls"
            >> beam.ParDo(ParseDataDoFn())  # Use the consolidated parsing function
        )
        ...
        parquet_pcoll = (
            ...
            >> beam.io.ReadFromParquet(
                "/Users/nicolas/repos/walmart/data/extra_data.parquet"
            )
        ...
        keyed_csv_pcoll = csv_pcoll | "Key By Index (CSV)" >> beam.Map(
            lambda x: (x["index"], x)
        )
        ...
        merged_pcoll = (
            ...
            | "Filter Out Empty Merged Records"
            >> beam.Filter(lambda x: x is not None)  # Remove None records
        )
        ...
        formatted_clean_pcoll = clean_pcoll | "Format to CSV" >> beam.Map(
            format_csv_row_clean
        )  # Convert each dictionary to CSV row
        formatted_clean_pcoll | "Write Clean CSV" >> beam.io.WriteToText(
            "/Users/nicolas/repos/walmart/data/csv_clean_dataflow",
            file_name_suffix=".csv",
            shard_name_template="",
        )

        # Write aggregated PCollection to CSV
        formatted_agg_pcoll = aggregated_pcoll | "Format to Agg CSV" >> beam.Map(
            format_csv_row_agg
        )  # Convert each dictionary to CSV row
        formatted_agg_pcoll | "Write Aggregated CSV" >> beam.io.WriteToText(
            "/Users/nicolas/repos/walmart/data/csv_aggregated_dataflow",
            file_name_suffix=".csv",
            shard_name_template="",
        )
