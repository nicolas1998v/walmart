...
"""
Walmart Sales Data Processing Pipeline. PySpark RDD's version.

This script processes Walmart sales data using Apache Spark's RDD API. It performs the following operations:
- Reads sales data from a SQLite database and additional data from Parquet files
- Cleans and transforms the data, including handling null values
- Joins sales data with store information
- Filters for high-value sales (>$10,000)
- Calculates monthly average sales
- Exports processed data to CSV files

The pipeline uses a class-based approach with WalmartSalesProcessor handling all data processing steps.
...
Input Files:
    ...
    - extra_data.parquet: Parquet file containing additional store information

Output Files:
    - clean_data_rdd.csv: Processed and filtered sales data
    - agg_data_rdd.csv: Monthly average sales calculations
"""
...
class WalmartSalesProcessor:
    ...
    def extract_month_from_flattened(record):
        key, values = record
        values["Month"] = values["Date"].month
        return (key, values)
    ...
    def process_data(self, database_path, parquet_path):
        ...
        extra_data_rdd = self.spark.read.parquet(parquet_path).rdd.map(
            lambda r: r.asDict()
        )
    ...
    def calculate_monthly_averages(self, filtered_rdd):
        month_sales_rdd = filtered_rdd.map(
            lambda x: (x[1]["Month"], x[1]["Weekly_Sales"])
        )
        grouped_sales_rdd = month_sales_rdd.groupByKey()
        return grouped_sales_rdd.mapValues(
            lambda sales: round(sum(sales) / len(sales), 2)
        )

    @staticmethod
    def save_rdds_as_csv(
        filtered_rdd, average_sales_rdd, filtered_rdd_path, average_sales_rdd_path
    ):
...
def main():
    ...
    filtered_rdd_csv = filtered_rdd.map(
        lambda x: ",".join(
            map(
                str,
                [
    ...
    # Save and validate
    processor.save_rdds_as_csv(
        filtered_rdd_csv, average_sales_rdd_csv, clean_data_path, agg_data_path
    )
